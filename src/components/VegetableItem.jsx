...
const VegetableItem = ({vegetables}) => {

    const {image , price , title } = vegetables

    const {dispatch} = useContext(CartContext)

    const handleAdd = (vegetables) =>{
            dispatch({
              type : "ADD_CART",
              payload : vegetables
            }) 
  ...
  return (
    ...
    <CardMedia  className='image-sec'
      ...
      image= {image}/>
      <button id='cart-btn' style={{position:"absolute",bottom:10,left:10,color:"white",border:"none",fontSize:12,padding:"1px 15px",borderRadius:10}}onClick={()=>handleAdd(vegetables)} >Buy</button>
  ...
  <CardContent sx={{display:"flex" , alignItems:"center",flexDirection:"column", justifyContent:"space-between",height:"130px"}} >
   ...
   <button style={{backgroundColor:"red",border:"none",padding:"1px 4px",borderRadius:20,marginLeft:"70px"}} sx={{bgcolor:"red" , borderRadius:10  }}><ShoppingBasketIcon sx={{color:"white" ,fontSize:15,mt:"2px"}}/></button>
      ...
      <Box sx={{display:"flex " , alignItems:"center",justifyContent:"center",height:"40px",flexDirection:"column",pl:"45px"}}>
